...
def accuracy_score(p1, p2, p3, true_classes):
    acc = 0
    for x, y, z, t in zip(p1, p2, p3, true_classes):
        count = 0
        if x == t:
            count += 1
        if y == t:
...
if __name__ == '__main__':
    ...
    test_set = class_0[int(len(class_0) * x2):] + \
        class_1[int(len(class_1) * x2):] + class_2[int(len(class_2) * x2):]
    test_x = [row[:-1] for row in test_set]
    test_y = [row[-1] for row in test_set]

    classifier_1 = GaussianNB()
    classifier_2 = DecisionTreeClassifier(random_state=0)
    classifier_3 = RandomForestClassifier(n_estimators=3, random_state=0)

    train_set_1 = class_0[:int(len(class_0) * x1)] + \
        class_1[:int(len(class_1) * x1)] + class_2[:int(len(class_2) * x1)]
    ...
    train_set_2 = class_0[int(len(class_0) * x1):int(len(class_0) * x2)] + \
        class_1[int(len(class_1) * x1):int(len(class_1) * x2)] + \
        class_2[int(len(class_2) * x1):int(len(class_2) * x2)]
    ...
    train_set_3 = class_0[:int(len(class_0) * x2)] + \
        class_1[:int(len(class_1) * x2)] + class_2[:int(len(class_2) * x2)]
    ...
    print(f'Tocnost: {accuracy_score(predictions_1,
          predictions_2, predictions_3, test_y)}')
