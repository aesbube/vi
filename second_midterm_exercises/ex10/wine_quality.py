...
if __name__ == '__main__':
    ...
    for row in dataset:
        if row[-1] > 5:
            updated_data.append(row[:-1] + [1])
        else:
            updated_data.append(row[:-1] + [0])

    train_set = updated_data[int(len(updated_data) * x):]
    train_x = [row[:-1] for row in train_set]
    train_y = [row[-1] for row in train_set]

    test_set = updated_data[:int(len(updated_data) * x)]
    ...
    least_impo = importances.index(min(importances))

    train_x = [[row[i]
                for i in range(len(row)) if i != least_impo] for row in train_x]
    test_x = [[row[i]
               for i in range(len(row)) if i != least_impo] for row in test_x]

    classifier_1 = MLPClassifier(hidden_layer_sizes=15, activation='relu',
                                 learning_rate_init=0.001, max_iter=200, random_state=0)
    classifier_2 = MLPClassifier(hidden_layer_sizes=15, activation='relu',
                                 learning_rate_init=0.001, max_iter=200, random_state=0)
    ...
    print(f'Tocnost so StandardScaler: {accuracy}')
    
    train_x = scaler_2.fit_transform(train_x)
    test_x = scaler_2.transform(test_x)
    classifier_2.fit(train_x, train_y)
    
    predictions = classifier_2.predict(test_x)
    accuracy = accuracy_score(predictions, test_y)
    print(f'Tocnost so MinMaxScaler: {accuracy}')