...
if __name__ == '__main__':
    ...
    train_set = class_0[:int(len(class_0)*percentage)] + \
        class_1[:int(len(class_1)*percentage)]

    train_x = [row[:-1] for row in train_set]
    train_y = [row[-1] for row in train_set]

    test_set = class_0[int(len(class_0)*percentage):] + \
        class_1[int(len(class_1)*percentage):]
    ...
    classifier_3 = RandomForestClassifier(
        criterion='entropy', n_estimators=4, random_state=0)
    classifier_4 = MLPClassifier(
        activation='relu', learning_rate_init=0.001, hidden_layer_sizes=10, random_state=0)
    ...
    predictions_1 = classifier_1.predict(test_x)
    predictions_2 = classifier_2.predict(test_x)
    predictions_3 = classifier_3.predict(test_x)
    predictions_4 = classifier_4.predict(test_x)

    acc_1 = accuracy_score(test_y, predictions_1)
    acc_2 = accuracy_score(test_y, predictions_2)
    acc_3 = accuracy_score(test_y, predictions_3)
    acc_4 = accuracy_score(test_y, predictions_4)

    accuracies = {"Naive Bayes": acc_1, "Decision Tree": acc_2,
                  "Random Forest": acc_3, "MLP": acc_4}

    max_classifier = max(accuracies.values())
    cls = ""
    for acc in accuracies.keys():
        if accuracies[acc] == max_classifier:
            cls = acc
            break

    print(f'Najgolema tocnost ima klasifikatorot {cls}')

    weights = [2 if clf == max_classifier else 1 for clf in [
